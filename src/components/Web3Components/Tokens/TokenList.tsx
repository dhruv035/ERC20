  ...
  return (
    <div className="w-full">
      {tokens.map((token: TokenData, index: number) => {
        return (
          <li
            className="flex flex-col items-center bg-transparent p-4 mb-2 first:rounded-t-2xl last:rounded-b-2xl last:mb-0 hover:cursor-pointer hover:bg-background md:flex-row"
            key={index}
            onClick={(e) => {
              setIsOpen(false);
              setToken(token);
          ...
          >
            <Image
              className="mb-4 w-[50px] rounded-full shadow-fuller shadow-gray-200"
